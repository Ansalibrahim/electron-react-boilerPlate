...
class WorldMap extends Component {
  ...
  handleCountryClick(countryIndex) {
    ...
    this.setState({
        focusedCountry: countryIndex,
        focusedCity: -1,
        focusedText: this.state.worlddata[countryIndex].id
    })
  }
  handleMarkerClick(markerIndex) {
    console.log("Marker: ", this.state.cities[markerIndex])
    this.setState({
        focusedCity: markerIndex,
        focusedCountry: -1,
        focusedText: this.state.cities[markerIndex].name
    })
  }
  componentDidMount() {
  ...
  handleMouseOverCountry=(countryIndex)=>{

  }
  render() {
    return (
    <div>
      <svg width={ '100%' } height={ '100%' } viewBox="0 0 800 450" ref={"svg"}>
        <g className="countries">
          {
            this.state.worlddata.map((d,i) => (
                    <path
                        ...
                        // className="country"
                        fill={ this.state.focusedCountry === i ?  '#057897' : '#a70a0a' }
                        // // stroke={this.state.focusedCountry === i ? "#FFF000":"#FFFFFF"}
                        // strokeWidth={ 0.5 }
                        onClick={ () => this.handleCountryClick(i) }
                        onMouseOver={()=>{
                            this.handleMouseOverCountry(i)
                        }}
                    />
            ))
          }
